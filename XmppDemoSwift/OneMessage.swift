...
public class OneMessage: NSObject {
    ...
    public class func sendMessageWithMedia(media: UIImage, to recipient: String, completionHandler completion:OneChatMessageCompletionHandler) {
        let body = DDXMLElement.elementWithName("body") as! DDXMLElement
        let messageID = OneChat.sharedInstance.xmppStream?.generateUUID()
        ...
        completeMessage.addChild(active)
        //IMAGEN TO DATA
        let imageData: NSData = UIImagePNGRepresentation(media)!
        let imageBase64 = imageData.base64EncodedStringWithOptions(.Encoding64CharacterLineLength)
        let xmlMedia = DDXMLElement.elementWithName("attachement", stringValue: imageBase64) as! DDXMLElement
        completeMessage.addChild(xmlMedia)
